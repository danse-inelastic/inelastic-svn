...
some of the molecules in our data set from Ref.~\cite{McClurg09}, such as
...
molecules are reduced to spheres (as opposed to dimers, etc.—see
...
\referencelist[thesisbibAltered,thesisbib]

\pagebreak

\begin{figure}
\caption{$[111]$ view of $T_d$
...
\begin{figure}
\caption{$[\bar{1}\bar{1}\bar{1}]$ view of the $T_d$ global phase
...
To couple $\{D_{m_i n_i}^{\ell_i},C_{m}^{\ell},D_{m_j
...
so are zero to first order. These coefficients can be calculated efficiently from \emph{ab
...
By symmetry adapting in two steps, we can define the
...
independent variables. The rotator functions
...
discussed in Ref.~\cite{Mettes04}.
