...
physics community, such as the Axial Next Nearest Neighbor Ising model (ANNNI),
which has been used for decades to explore complex phase
diagrams\cite{Gendiar05, Kasama06, Muraoka99, Sato99, Kim95}

Global phase diagrams as a function of first or second nearest neighbor
interactions $J_1$ or $J_2$ or external field are typically the parameters
involved.

Although these have some relation to the model we present here, we point out
their potential is not a complete set of basis functions over SO(3).



Like C's fluid-phase GPDs, molecular crystal
...
interesting to compare the results from this final work with magnetic materials modeled
using, for example, the ANNN{1,2,3}-type models which have had some
success reproducing real materials(ref) despite their primitive
potential. 
