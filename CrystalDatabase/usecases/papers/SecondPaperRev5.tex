...
high-temperature reference lattice, such as that shown at the origin of
Figs.~\ref{gpd1} and~\ref{gpd2}, in which molecules remain translationally ordered but are
rotationally disordered. Experimentally this is termed a plastically
crystalline state \cite{Sherwood79} and is observable for some of the molecules
in our data set from \cite{McClurg09}, such as adamantane or methane.  Other systems melt, sublime, or decompose before the plastically-crystalline state is obtained.  Whether or not the reference state
...
An analog in the physics community to to molecular crystal GPDs are Ising-type
GPDs, such as the Axial Next Nearest Neighbor Ising model (ANNNI),
which has been used for decades to explore both commensurate and incommensurate
phases\cite{Gendiar05, Kasama06, Muraoka99, Sato99, Kim95} that can be applied to complex materials such as silicon carbide, titanium aluminum, group-IV element
polytypes, and so on\cite{Wong-Leung05, Colinet02, Raffy02}. These differ from
molecular crystal GPDs in that their basis functions are simple binary variables $S_i\in \{1,-1\}$ coupled by a constant $J$, although such couplings
typically exist with next-nearest neighbors or larger.

However, like C's fluid-phase GPDs, molecular crystal
...
temperature-dependent GPDs is needed and is underway. It would also be
interesting to compare the results from this final work with similar
