...
Despite their much simpler potentials, these have been used for decades to
explore a rich variety of commensurate and incommensurate
...
element polytypes, and so on\cite{Wong-Leung05,Colinet02,Raffy02}. These differ
from molecular crystal GPDs in that their basis functions are simple binary
variables $S_i\in \{1,-1\}$ coupled by a constant $J$, although such couplings
typically exist with next-nearest neighbors or larger. However, it will be
interesting to compare the results of these simplified models with a full
molecular crystal GPD now that we have established a sufficiently large
parameter space.

A further extension would be to include the intersection of each
solid GPD with its set of high-temperature fluid GPD.  This
would be useful in to crystallographers wishing to predict the first solid
phase they would expect to precipitate from melt.  Furthermore,
liquidlocate the high temperature intersection of each ray with its
liquid phase
