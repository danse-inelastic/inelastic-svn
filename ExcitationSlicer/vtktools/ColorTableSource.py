...
"""Class for generating a colortable

This class is based on 'vtkLookupTable' and can be used to generate 
colortables. The methods of 'vtkLookupTable' can be accessed directly. In 
addition to these methods it is possible to generate a colortable via 
'SetHSVColors' and 'SetRGBColors' .
"""
import sys
try:
...
# NOTE: Using the recipe as described by the VTK mailling list to construct
#       a Python class that is based on a VTK c++ class. 
#       ColorTableSource is recognized as a VTK class derived from
#       vtkLookupTable

class ColorTableSource:

	def __init__(self):
             self.__dict__['__vtkobject__'] = vtkLookupTable()
             self.SetHSVColors()
	...
	def SetHSVColors(self,huerange=(0.75,0.0),valuerange=(1.0,1.0)):
		"""Sets the vtkLookupTable

		This method can be used to change the colortable. The 
		colortable uses the HSV system to represent the colors. 
		The hue represents the dominant wavelength of a color.
		The value represents how much light is in the color - 0
		thus corresponds to black. 

		**An example** 
	
		To generate a colortable with the hue range from 0 to 0.33 
		use: 'SetHSVColors(huerange=(0,0.33))' . This will result 
		in a colortable ranging from red to green.
		"""
		# Allocating a new empty colortable
		self.Allocate(256,256)
		self.SetHueRange(tuple(huerange))
		self.SetValueRange(tuple(valuerange))
		self.Build()
		

	def SetRGBColors(self,colorlist):
		"""Sets the vtkLookupTable

		This method can be used to change the colortable. 'colorlist'
		is expected to be a list of colors whose elements is either 
		specified in terms of the RGB colorscale or in terms of the 
		name of a color. A complete list of the recognized colors can 
		be obtained via the method 'GetColorNames' . 

		**An example**

		To generate a colortable going from red to 
		(0.5000,0.1647,0.1647) to black use: 
		'SetRGBColors(["red",(0.5000,0.1647,0.1647),"black"])' .
		"""
		from Numeric import asarray,Float,array

		interval=256/(len(colorlist)-1)
		Ncolors=interval*(len(colorlist)-1)

		self.SetNumberOfColors(Ncolors)

		# Translating colors (if any) to r,g,b numbers
		for i in range(len(colorlist)):
			if type(colorlist[i]) in [type([]),type(())]:
				colorlist[i]=asarray(colorlist[i],Float)
			else:
				colorlist[i]=array(self.ColorNameToRGB(colorlist[i]))
				
		# Finding the difference between adjacent colors
		# dcolorlist is the steplenght for each color entry
		dcolorlist=[]

		for i in range(len(colorlist)-1):
			dcolorlist.append((colorlist[i+1]-colorlist[i])/interval)
		# Inserting the colors in the table
		color=colorlist[0]
		for loopnumber in range(len(colorlist)-1):
			for entry in range(loopnumber*interval,(loopnumber+1)*interval):
				r,g,b=color
				self.SetTableValue(entry,r,g,b,1.0)
				color=color+dcolorlist[loopnumber]

	def ColorNameToRGB(self,colorname):
		"""Returns a tuple

		This method can be used to translate a 'colorname' into its
		corresponding RGB value.
		"""
		if colorname in self.GetColorNames():
			return self.__ColorSource[colorname]
		else:
			raise NameError, 'The color '+colorname+' is not defined'

	def GetColorNames(self):
		"""Returns a list of the recognized color names"""
		return self.__ColorSource.keys()
