...
/* $Id: pto2D.cpp,v 1.3 2007/05/17 18:16:14 emessick Exp $ */
...
pto2D::pto2D (float xt, float yt)
{
    x = xt;
    y = yt;
    z = 0.0f;
}
pto2D::pto2D (double xt, double yt)
{
    x = (float) xt;
    y = (float) yt;
    z = 0.0f;
}
pto2D::pto2D (pto3D other)
{
    x = other.x;
    y = other.y;
    z = 0.0f;
...
boolean pto2D::dentro3l (pto2D A, pto2D B, pto2D C)	//el metodo laxo
{
...
boolean pto2D::dentro4 (pto2D A, pto2D B, pto2D C, pto2D D, double p)
{
    ...
    if (a1 < margen || a1 > M_PI - margen) {
	nconcav++;
	ac[0] = true;
    } else
	ac[0] = false;
    if (a2 < margen || a2 > M_PI - margen) {
	nconcav++;
	ac[1] = true;
    } else
	ac[1] = false;
    if (a3 < margen || a3 > M_PI - margen) {
	nconcav++;
	ac[2] = true;
    } else
	ac[2] = false;
    if (a4 < margen || a4 > M_PI - margen) {
	nconcav++;
	ac[3] = true;
    } else
	ac[3] = false;
...
boolean pto2D::dentro4 (pto2D A, pto2D B, pto2D C, pto2D D)
{
    return dentro4 (A, B, C, D, 0.0);
}


double pto2D::anguloccwhasta (pto2D phasta)	//EN RADIANES
{
    double an = 0;
...
double pto2D::angulocwhasta (pto2D phasta)	//EN RADIANES
{
    double an = 0;
    double a = anguloconr (phasta);
    pto3D pv = prodvect (phasta);
...
boolean pto2D::dentro4e (pto2D A, pto2D B, pto2D C, pto2D D)
{
    return dentro4 (A, B, C, D, 1.00);
}			//estricto

boolean pto2D::dentro4l (pto2D A, pto2D B, pto2D C, pto2D D)
{
    return dentro4 (A, B, C, D, 1.01);
}			//laxo

pto2D pto2D::mas (pto2D p)
{
    return pto2D (x + p.x, y + p.y);
}
pto2D pto2D::menos (pto2D p)
{
    return pto2D (x - p.x, y - p.y);
}
