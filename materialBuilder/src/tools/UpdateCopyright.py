...
copyrightRegex = re.compile(r'Copyright\s+(?:\(c\))?\s*(\d{1,4}(\s*(\,|\-)\s*\d{1,4})*)\s+materialBuilder\,\s+Inc\.')

# This regex recognizes the @copyright: epydoc field.
epydocCopyrightRegex = re.compile(r'^(.*\@copyright\:\s*)(\d{1,4}(\s*(\,|\-)\s*\d{1,4})*)(\s+materialBuilder\,\s+Inc\..*)')
...
def writeCopyrightLine(f, path, dateString):
    ...
    f.write("%s Copyright %s Brandon Keith  See LICENSE file for details. %s\n" % (commentStart, dateString, commentEnd))
