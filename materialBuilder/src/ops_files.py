...
$Id: ops_files.py,v 1.75 2007/07/04 03:23:42 ninad Exp $
...
#                   env.history.message( cmd + "PDB file inserted: [ " + os.path.normpath(import_filename) + " ]" )
                ...
                tmpdir = platform.find_or_make_materialBuilder_subdir('temp')
...
##            print "done time =",self.__last_closeEvent_cancel_done_time
    ...
    def _updateRecentFileList(self, fileName):
        ...
        if recentfiles_use_QSettings:
            ...
            prefsSetting.setValue('/materialBuilder/nE-1/recentFiles', QVariant(fileList))
            if debug_recent_files:
                # confirm that the information really made it into the QSetting.
                fileListTest = prefsSetting.value('/materialBuilder/nE-1/recentFiles').toStringList()
                fileListTest = map(str, list(fileListTest))
                assert len(fileListTest) == len(fileList)
                for i in range(len(fileList)):
                    assert str(fileList[i]) == str(fileListTest[i])
        else:
            prefsSetting['/materialBuilder/nE-1/recentFiles'] = fileList 
...
if __name__ == '__main__':
    ...
    prefs.writeEntry('/materialBuilder/nE-1/recentFiles', emptyList)
