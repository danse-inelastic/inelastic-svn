...
$Id: platform.py,v 1.50 2007/07/01 17:27:32 emessick Exp $
"""
...
def find_or_make_materialBuilder_directory():
    """
    Find or make the directory ~/materialBuilder, in which we will store
    ...
    _tmpFilePath = _find_or_make_materialBuilder_dir_0()
    assert _tmpFilePath
    return _tmpFilePath

def _find_or_make_materialBuilder_dir_0():
    """private helper function for find_or_make_materialBuilder_directory"""
    ...
    #   enough test.
    tmpFilePath = os.path.normpath(os.path.expanduser("~/materialBuilder/"))
...
def path_of_materialBuilder_subdir(subdir): #bruce 060614
    """Return the full pathname which should be used for the given ~/materialBuilder subdirectory,
    without checking whether it exists.
       WARNING: as a kluge, the current implem may create ~/materialBuilder itself.
    ...
    """
    materialBuilder = find_or_make_materialBuilder_directory()
    materialBuilder_subdir = os.path.join(materialBuilder, subdir)
    return materialBuilder_subdir
    
def find_or_make_materialBuilder_subdir(subdir, make = True): #bruce 060614 added make arg; revised implem (so subdir can be >1 level deep)
    """Find or make a given subdirectory under ~/materialBuilder/. It's allowed to be more than one level deep, using '/' separator.
    ...
    Return the full path of the materialBuilder subdirectory, whether it already exists or was made here.
    """
    subdir = path_of_materialBuilder_subdir(subdir)

    errorcode, path_or_errortext = find_or_make_any_directory(subdir, make = make)
    if errorcode:
        if make:
            # this should not normally happen, since ~/materialBuilder should be writable, but it's possible in theory
...
def user_plugins_dir():
    ...
    return find_or_make_materialBuilder_subdir( 'Plugins', make = False)
...
def make_history_filename():
    ...
    """
    prefsdir = find_or_make_materialBuilder_directory()
    tried_already = None
    while 1:
        timestamp_string = time.strftime("%Y%m%d-%H%M%S")
        histfile = os.path.join( prefsdir, "Histories", "h%s.txt" % timestamp_string )
            # e.g. ~/materialBuilder/Histories/h20050104-160000.txt
        ...
        # (in the same materialBuilder-specific prefs directory) must have started less
...
def tempfiles_dir(make = True): #bruce 060614
    ...
    e.g. from ~/materialBuilder/TemporaryFiles into ~/materialBuilder/OldTempFiles,
    ...
    _tempfiles_dir = find_or_make_materialBuilder_subdir("TemporaryFiles/t%s" % _histfile_timestamp_string, make = make )
...
def move_tempfiles_dir_when_quitting(): #bruce 060614 ###@@@ need to call this when nE-1 quits
    ...
    movetodir = find_or_make_materialBuilder_subdir("OldTempFiles")
