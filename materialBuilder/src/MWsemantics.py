...
$Id: MWsemantics.py,v 1.475 2007/08/02 15:03:50 ninad Exp $
...
class MWsemantics(QMainWindow, fileSlotsMixin, viewSlotsMixin, movieDashboardSlotsMixin, Ui_MainWindow):
	...
	self.viewMenu.addMenu(toolbarMenu)
        ...
        self.tmpFilePath = platform.find_or_make_materialBuilder_directory()
        ...
        windowList += [self]
        if name == None:
            self.setWindowTitle("Sample Builder") # Mark 11-05-2004
    ...
    def init_part_two(self):
        ...
        if recentfiles_use_QSettings:
            prefsSetting = QSettings("materialBuilder", "Sample Builder")
        else:
            prefsSetting = preferences.prefs_context()
	    
        if recentfiles_use_QSettings:
            # Qt3: fileList = prefsSetting.readListEntry('/materialBuilder/nE-1/recentFiles')[0]
            fileList = prefsSetting.value('/materialBuilder/nE-1/recentFiles').toStringList()
        else:
            fileList = prefsSetting.get('/materialBuilder/nE-1/recentFiles', [])
        if len(fileList):  
...
#        msgbox.show()
    ...
    def helpAbout(self):
        """Displays information about this version of Sample Builder
        """
	...
	if filePath.endswith('/Contents/Resources'):
        ...
        techsupport = "For technical support, send email to support@materialBuilder.com"
        ...
        QMessageBox.about ( self, "About Sample Builder", aboutstr)
...
# == Caption methods
    ...
    def update_mainwindow_caption(self, changed = False): #by mark; bruce 050810 revised this in several ways, fixed bug 785
        ...
        Example:  "Sample Builder - [partname.mmp]"
	...
	self.setWindowTitle(self.trUtf8("Sample Builder" + " - " +prefix + "[" + partname + "]" + suffix))
