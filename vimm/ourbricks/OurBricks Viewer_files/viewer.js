...
Kata.userRenderCallback = function(t){
    ...
    if (!(typeof(Kata_showfps) == "undefined")) {
        if (Kata_showfps == "console") {
            console.log("fps:", fps)
        }
        else {
...
setGroundPlaneHeight=function(v,groundPlaneMesh) {
    if (window.kata) {
        window.kata.getChannel().sendMessage(new Kata.ScriptProtocol.ToScript.GUIMessage({
            msg: "groundHeight",
            event: {
...
setSkybox=function(skybox,sunbeams){
    if (window.kata) {
        window.kata.getChannel().sendMessage(new Kata.ScriptProtocol.ToScript.GUIMessage({
                                                                                         msg:"skybox",
                                                                                         event:{value:skybox?["skydome.png"]:[],
                                                                                               sunbeams:sunbeams}}));
    }
};
    ...
    if (cookie && uid && document.getElementById("preview_extra_html")) {
        checkIfUser(cookie,
            function(status){
                ...
                Kata.log("Error loading Document: status "+ status);
                main1();
            },
            function(resptext){
...
function enablePreview(){
    ...
    var preview_html_text = '\
        <div style="position:absolute;left:10px;bottom:-22px;color:white">\
            scale:\
        </div>\
        <div style="position:absolute;left:300px;bottom:-22px;color:white">\
            ground:\
        </div>\
        <div style="position:absolute;left:600px;bottom:-25px;color:white">\
            <div style="float:left;margin-right:10px">\
                shadows:\
                <button id="toggle_shadows_button" onclick = "toggleShadows('+"'"+'toggle_shadows_button'+"'"+')" >Off</button>\
            </div>\
            <div style="float:left;margin-right:10px">\
                sunbeam:\
                <button id="toggle_sunbeam_button" onclick = "toggleSunbeams('+"'"+'toggle_sunbeam_button'+"'"+','+"'"+'toggle_skybox_button'+"'"+')" >Off</button>\
            </div>\
            <div style="float:left;margin-right:10px">\
                skybox:\
                <button id="toggle_skybox_button" onclick = "toggleSkybox('+"'"+'toggle_skybox_button'+"'"+','+"'"+'toggle_sunbeam_button'+"'"+')" >Off</button>\
            </div>\
            <div style="float:left;margin-right:10px">\
                mouse rotation:\
                <button id="toggle_rotation_button" onclick = "toggleMouseRotation()" >On</button>\
            </div>\
        </div>\
    ...
    document.getElementById("preview_extra_html").innerHTML = preview_html_text;
                    if (extra.mouse_rot!==undefined && extra.mouse_rot == false) {
                        if (document.getElementById("toggle_rotation_button")) {
                            toggleMouseRotation();
                        }
                    }
                    if (extra.shadows!==undefined && extra.shadows) {
                        if (document.getElementById("toggle_shadows_button")) {
                            toggleOnOff("toggle_shadows_button");
                        }
                    }
                    if (extra.skybox!==undefined && extra.skybox.length) {
                        if (document.getElementById("toggle_skybox_button")) {
                            toggleOnOff("toggle_skybox_button");
                        }
                    }
                    if (extra.sunbeams!==undefined && extra.sunbeams) {
                        if (document.getElementById("toggle_sunbeam_button")) {
                            toggleOnOff("toggle_sunbeam_button");
                        }
                    }

    createRescaleSlider();
    createGroundPlaneSlider();
    extra.scale = temp;
    if (extra.errors) {
...
function handleIframeMessage(msg){
    ...
    if (data.__type==Kata.ScriptProtocol.FromScript.Types.GraphicsMessage) {
	
	g_graphics.mGFX.send(data);
    }else{
	
	window.kata.getChannel().sendMessage(data);
    }
    if (data.__type!==undefined)
	document.getElementById('watermark').style.display="none";//turn off watermark in noniframe display
}
        ...
        if (req) {
            ...
            req.onreadystatechange = function(){
                ...
                if (this.readyState == 4) {
                    ...
                    // always disable edit button
                    if (true || !g_allowPreview) {
                    ...
                    window.parent.postMessage(JSON.stringify(extra),"*");
                    console.log("mainscene load:", load);
                    var loadargs = {
                        mesh: load,
                        groundHeight : extra.groundHeight,
                        groundMesh : extra.groundMesh,
                        skybox : extra.skybox,
                        sunbeams : extra.sunbeams,
                        shadows : extra.shadows,
                        anim: extra.anim,
                        up_axis: extra.up_axis,
                        scale: extra.scale,
                        mouse_rot: mouse_rot,
                        center: extra.center,
                        bounds: (extra.bounds.length==3?[0,0,0,extra.bounds[2]]:extra.bounds),
                        driver: g_driver
                    };
                    if (extra.pos) {
...
//                            Kata.warn("moved to " + msg.data.campos);
                            if (msg.data.type == "DownloadComplete") {//formerly loaded
                                GlobalLoadDone();
                            }
                            else if (msg.data.type == "Loaded") {//formerly loaded
                                //GlobalLoadDone();
                            }
                            else if (msg.data.type == "canvasCapture") {
				msg.data.objPos=g_objPos;
				msg.data.objOrient=g_objOrient;
				msg.data.camPos=g_camPos;
				msg.data.camOrient=g_camOrient;
				msg.data.loadUrl=g_loadUrl;
				window.parent.postMessage(JSON.stringify(msg.data),"*");
                                //publishNowFinish(msg.data.img);
                            }
                            else if (msg.data.type=="CreatedLoadObject") {
				
			    }
			    else if (msg.data.campos !== undefined) {
                ...
                if (req.readyState == 4) {
                    if (req.status != 200 && req.status != 0) {
                        ...
                        if (req.responseText.indexOf("ERROR") == 0) {
                            Kata.log(req.responseText);
                        }
                        else {
                            //alert (req.responseText.substr(req.responseText.indexOf("SUCCESS:")+8))
                            if (req.responseText.indexOf("ERROR") >= 0) {
                                Kata.log("Response error :"+req.responseText);
                            }
                            else {
