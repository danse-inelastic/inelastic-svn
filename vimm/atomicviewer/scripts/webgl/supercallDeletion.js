...
var g_material;
var g_zoomFactor;
var g_treeInfo = true;  // information about the transform graph.
...
//var tableAtom = [['Fe', [0.5,0.5,0.5]], ['C', [2.5,2.5,2.5]], ['C', [3.5, 3.5, 3.5]]];
//var tableBond = [ [[0.5,0.5,0.5],[1.5,1.5,1.5]], [3.5, 3.5, 3.5]];
...
function init(lattice, atoms, phonon) {
		...
		for (var indexY = 0; indexY < sizeY; indexY++) {//duplicate the cell along the Y axis
			...
			for(var indexZ=0; indexZ<sizeZ; indexZ++){
				...
				for(var indexTableBond=0; indexTableBond<tableBond.length; indexTableBond++){
					...
					var bond=g_mainPack.createObject('Transform');
...
function drawlattice(tableLattice, tableAtoms, tableBonds, sizeX, sizeY, sizeZ){
	...
	for (var indexTableElements = 0; indexTableElements < transformTableElements.length; ++indexTableElements) {
		var transformEle = g_mainPack.createObject('Transform');
	...
	for (var indexTableBonds = 0; indexTableBonds < transformTableBonds.length; ++indexTableBonds) {
		var transformBonds = g_mainPack.createObject('Transform');
	...
	for (var indexTableSupercell = 0; indexTableSupercell < transformTableSupercell.length; ++indexTableSupercell) {
		var transformSupercell = g_mainPack.createObject('Transform');
...
function createSphereShapeBySymbol(symbol){
	...
	var sphere = o3djs.primitives.createSphere(g_mainPack,
												createMaterial(colorOfElement),
												radiusOfElement*(1/5),   // Radius of the sphere.
...
function createShapeForSupercellLine(point1,point2){
	...
	var material=o3djs.material.createConstantMaterial(g_mainPack,
														g_viewInfo,
														colorOrTexture);
	var vertices = [
  ...
  if(state%2==0)
      {
          if(counter<300)
              ...
              {
                  for(i = 3;i<tablePhonon[0]+3;i++)
                     {
                       tableAtom[i-3] = [tablePhonon[i][0], [(tablePhonon[i][1]+tablePhonon[i][4]),(tablePhonon[i][2] + tablePhonon[i][5]),(tablePhonon[i][3] + tablePhonon[i][6])]]
                     }
                  deleteLattice(sizeX, sizeY, sizeZ, tableLattice, tableAtom, atom)
  ...
  else if(state%2==1)
      {
          if(counter<300)
          ...
          else
              {
                  ...
                  for(i = 3;i<tablePhonon[0]+3;i++)
                     {
                       tableAtom[i-3] = [tablePhonon[i][0], [tablePhonon[i][1],tablePhonon[i][2],tablePhonon[i][3]]]
                     }
                  deleteLattice(sizeX, sizeY, sizeZ, tableLattice,  tableAtom, atom)
                  //createLattice(sizeX, sizeY, sizeZ, tableLattice, tableAtom)
                  counter = 0;
