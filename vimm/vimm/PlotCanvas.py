...
import wx
from vimm.NumWrap import NumWrap
...
class PolyPoints:
    ...
    def __init__(self, points, attr):
        self.points = NumWrap.array(points)
    ...
    def boundingBox(self):
        return NumWrap.minimum.reduce(self.points), \
               NumWrap.maximum.reduce(self.points)
...
class PlotGraphics:
    ...
    def boundingBox(self):
        ...
        for o in self.objects[1:]:
            ...
            p1 = NumWrap.minimum(p1, p1o)
            p2 = NumWrap.maximum(p2, p2o)
        return p1, p2
...
class PlotCanvas(wx.Window):
    ...
    def _setsize(self):
        ...
        self.plotbox_size = 0.97*NumWrap.array([self.width, -self.height])
        xo = 0.5*(self.width-self.plotbox_size[0])
        yo = self.height-0.5*(self.height+self.plotbox_size[1])
        self.plotbox_origin = NumWrap.array([xo, yo])
    ...
    def draw(self, graphics, xaxis = None, yaxis = None, dc = None):
        ...
        if yaxis is not None:
            ...
            yticks = None
        text1 = NumWrap.array([text_width[0], -text_height[1]])
        text2 = NumWrap.array([text_width[1], -text_height[0]])
    ...
    def _axisInterval(self, spec, lower, upper):
        ...
        if spec == 'automatic':
            ...
            if range == 0.:
                return lower-0.5, upper+0.5
            log = NumWrap.log10(range)
            power = NumWrap.floor(log)
            fraction = log-power
            if fraction <= 0.05:
                power = power-1
            grid = 10.**power
    ...
    def _drawAxes(self, dc, xaxis, yaxis,
        ...
        if xaxis is not None:
            ...
            for y, d in [(bb1[1], -3), (bb2[1], 3)]:
                p1 = scale*NumWrap.array([lower, y])+shift
                p2 = scale*NumWrap.array([upper, y])+shift
                dc.DrawLine(p1[0],p1[1],p2[0],p2[1])
                for x, label in xticks:
                    p = scale*NumWrap.array([x, y])+shift
        ...
        if yaxis is not None:
            ...
            for x, d in [(bb1[0], -3), (bb2[0], 3)]:
                p1 = scale*NumWrap.array([x, lower])+shift
                p2 = scale*NumWrap.array([x, upper])+shift
                dc.DrawLine(p1[0],p1[1],p2[0],p2[1])
    ...
    def _ticks(self, lower, upper):
        ideal = (upper-lower)/7.
        log = NumWrap.log10(ideal)
        power = NumWrap.floor(log)
        fraction = log-power
        factor = 1.
        error = fraction
        for f, lf in self._multiples:
            e = NumWrap.fabs(fraction-lf)
            if e < error:
                error = e
                factor = f
        ...
        ticks = []
        t = -grid*NumWrap.floor(-lower/grid)
        while t <= upper:
            ticks.append( (t, format % (t,)) )
            t = t + grid
        return ticks

    _multiples = [(2., NumWrap.log10(2.)), (5., NumWrap.log10(5.))]
...
class PlotDialog(wx.Dialog):
    def __init__(self,parent,id,title,objects):
        wx.Dialog.__init__(self,parent,id,title,
                          wx.PyDefaultPosition, wx.Size(400, 400))
        sizer = wx.BoxSizer(wx.VERTICAL)
        self.SetSizer(sizer)
        
        #self.objects = objects
        self.canvas = PlotCanvas(self)
        self.canvas.draw(objects,'automatic','automatic')
        #sizer.Add(canvas)
...
class PlotFrame(wx.Frame):
    def __init__(self, parent, id, title,objects):
        ...
        self.objects = objects
        self.canvas = PlotCanvas(self)
        self.canvas.draw(self.objects,'automatic','automatic')
        return

class MenuPlotFrame(wx.Frame):
    def __init__(self, parent, id, title,objects):
        wx.Frame.__init__(self, parent, id, title,
                          wx.PyDefaultPosition, wx.Size(400, 400))
    ...
    def OnFilePrint(self, event):
        d = wx.MessageDialog(self,
                             "As of this writing, printing support "
...
def test_objects():
    ...
    data1 = 2.*NumWrap.pi*NumWrap.arange(200)/200.
    data1.shape = (100, 2)
    data1[:,1] = NumWrap.sin(data1[:,0])
    markers1 = PolyMarker(data1, color='green', marker='circle',size=1)
    
    # 50 points cos function, plotted as red line
    data1 = 2.*NumWrap.pi*NumWrap.arange(100)/100.
    data1.shape = (50,2)
    data1[:,1] = NumWrap.cos(data1[:,0])
...
def test_app():
    app = wx.PySimpleApp()
    frame = PlotFrame(None,-1,"PlotCanvas",test_objects())
    frame.Show()
    app.MainLoop()
    return
...
def test_dialog():
    app = wx.PySimpleApp()
    d = PlotDialog(None,-1,"PlotDialog",test_objects())
    d.ShowModal()
    d.Destroy()
    app.MainLoop()
    return
