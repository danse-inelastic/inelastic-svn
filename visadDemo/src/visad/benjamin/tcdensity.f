...
C WLH
C        if(first) then			! Reconstruct spiral arm axes
        if (ii .eq. 1) then
          do 20 j=1,4
          dth=5.0/r1(1,j)
...
c
	...
	sech2=0.0
	if(abs(z).lt.10.0) sech2=(2.0/(exp(z)+exp(-z)))**2
	return
	end
