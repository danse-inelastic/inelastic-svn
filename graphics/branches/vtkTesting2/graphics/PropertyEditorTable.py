...
class MatplotlibPropertyTable(gridlib.PyGridTableBase):
...
#        self.dataTypes = [gridlib.GRID_VALUE_STRING]
    ...
    def loadModel(self,model):
        ...
        if self.backend=='matplotlib':
            if model.find('axes')!=-1:
                from graphics.MatplotlibPropertyModel import axesProps as properties, axesPropDataTypes as dataTypes
            elif model.find('line')!=-1:
                from graphics.MatplotlibPropertyModel import linePropDataTypes as dataTypes
                plotItem = gca()._prop['plotitems'][0]
                lineProps = plotItem._prop.items()
                ...
                from graphics.MatplotlibPropertyModel import figureProps as properties, figurePropDataTypes as dataTypes
            else:
                raise 'cannot find model properties'   
            self.properties = properties.sort()
            dataTypes=dataTypes.items()
            self.dataTypes = dataTypes.sort()
    ...
    #        attributes=[None for p in self.properties]
            # get the most current value if this is matplotlib
            if self.backend=='matplotlib':
                for i in range(len(self.properties)):
            ...
            import graphics.VtkPropertyModel
            ...
            if model.find('axes')!=-1:
                ...
                from graphics.VtkPropertyModel import plottableProps as properties 
            elif model.find('figure')!=-1:
