...
from graphics.api.GraphicsAPI import GraphicsAPI
from pylab import *

class MatplotlibEngine(GraphicsAPI, matplot):
    '''Graphics engine by Matplotlib '''
    # currently assumes a single plot window
    sessiontype = 'matplotlib'
    ...
    def xrange(self,min=0,max=1):
        xlim(min,max)

    # set the range for the y-axis
    def yrange(self,min=0,max=1):
        ylim(min,max)

    # toggle the x-axis to logscale
    def xlogscale(self,status=False,base=10):
        '''status can be True|False and base can be 10''' 
        value={False:'linear',True:'log'}
        gca().set_xscale(value[status])

    # toggle the y-axis to logscale
    def ylogscale(self,status=False,base=10):
        '''status can be True|False and base can be 10'''
        value={False:'linear',True:'log'}
        gca().set_xscale(value[status])
