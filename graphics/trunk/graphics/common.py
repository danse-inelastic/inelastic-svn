...
from graphics.numpytools import seq, iseq, asarray, NewAxis, ones, zeros, \
     sqrt, shape, NumPyArray, arrmin, arrmax, ravel, meshgrid, rank, squeeze, \
     reshape, compress

from graphics.misc import _check_xyz, _check_xyuv, _check_xyzuvw, _check_xyzv, \
...
class BaseClass(object):
    ...
    def hardcopy(self, filename='tmp.eps'): 
        """
        Save a hardcopy of the current figure to file (with the given
        filename).
        The format of the file (image type) is determined from the
        filename extension. If the format is not supported, PostScript
        output will be used.
        """
