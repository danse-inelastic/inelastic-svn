...
class VtkBackend(BaseClass):
    ...
    def _create_Tk_gui(self):
        ...
        fig._root.title("Graphics - Figure %d" % \
                        self._attrs['curfig'])
    ...
    def _create_wx_gui(self):
        ...
        self.wxRW = wxVTKRenderWindowWrapper(frame, -1)#, size=(500,500)) # use the main frame as the parent 

        #self.ren = vtk.vtkRenderer()
        #wx.GetRenderWindow().AddRenderer(self.ren)
        #self.wxRW.pack(expand='true', fill='both')
        renwin = self.wxRW.GetRenderWindow()
