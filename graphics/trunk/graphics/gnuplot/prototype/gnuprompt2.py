...
#           exec 'attr = self.Gnuplot.'+name
    ...
    def put(self,name,val):
        ...
        if isinstance(val,int) or\
           ...
           isinstance(val,long): pass
        else:
           print " invalid GNUPLOT expression "
           print " %s not set within GNUPLOT " % name
           return
...
#               if numerix:
        ...
        try: #if intended for python
            ...
            if com.count('='):
                ...
                if not name.count('['):
                    outlist.append(name)
                    value = com.split('=')[-1].strip()
                    exec 'val = %s' % value
                    if isinstance(val,int) or\
                       isinstance(val,float) or\
                       isinstance(val,long): 
                           self.session(com) #set vars within gnuplot
                    else:
                        print "         invalid GNUPLOT expression "
                        print "         %s not set within GNUPLOT " % name
        except:
